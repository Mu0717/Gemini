...
function addLinkVerifyLog(message, type = 'info', verificationId = null) {
  ...
  if (verificationId) {
    ...
    row.innerHTML = `<span class="vlog-col vlog-col-time">${timeStr}</span><span class="vlog-col vlog-col-id" title="${escapeHtml(
      verificationId
    )}" onclick="copyToClipboard('${escapeHtml(verificationId)}')">${escapeHtml(
      shortId
    )}</span><span class="vlog-col vlog-col-result"><span class="vlog-badge ${
      badge.cls
    }">${
      badge.text
    }</span></span><span class="vlog-col vlog-col-msg">${escapeHtml(
      message
    )}</span>`
  } else {
    // 系统消息行（不含验证ID的通用消息）
    row.className = 'vlog-row vlog-system'
    row.innerHTML = `<span class="vlog-col vlog-col-time">${timeStr}</span><span class="vlog-sys-msg ${type}">${escapeHtml(
      message
    )}</span>`
  }
...
async function verifySheerIDLink() {
  ...
  try {
    addLinkVerifyLog('📡 正在发送单次验证请求并获取动态状态...', 'info')
    showToast('正在验证...', 'info')
    const result = await api.post('/api/sheerid/verify_single', {
      verification_id: vid,
      api_key: apiKey,
    })

    addLinkVerifyLog(`📨 收到服务器最终响应`, 'info')

    if (result.success) {
      const res = result.result
      if (res && (res.status === 'success' || res.currentStep === 'success')) {
        ...
        if (res?.raw_data) {
          addLinkVerifyLog(
            `📝 错误详情: ${JSON.stringify(res.raw_data)}`,
            'warning'
          )
        }
...
function updateAutoSelectedCount() {
  ...
  if (selectAllCheckbox) {
    ...
    selectAllCheckbox.checked =
      selectableCount > 0 && autoState.selectedAccounts.size === selectableCount
  }
}
  ...
  try {
    ...
    if (result.success) {
      ...
      document.getElementById(
        'auto-progress-text'
      ).textContent = `${result.processed} / ${result.total}`
    ...
    if (result.success) {
      ...
      const percent =
        result.total > 0
          ...
          : 0
      document.getElementById(
        'sheerlink-progress-bar'
      ).style.width = `${percent}%`
      document.getElementById(
        'sheerlink-progress-text'
      ).textContent = `${result.processed} / ${result.total}`
    ...
    if (result.success) {
      ...
      const percent =
        result.total > 0
          ...
          : 0
      document.getElementById(
        'bindcard-progress-bar'
      ).style.width = `${percent}%`
      document.getElementById(
        'bindcard-progress-text'
      ).textContent = `${result.processed} / ${result.total}`
    ...
    if (result.success) {
      ...
      const percent =
        result.total > 0
          ...
          : 0
      document.getElementById(
        'change2fa-progress-bar'
      ).style.width = `${percent}%`
      document.getElementById(
        'change2fa-progress-text'
      ).textContent = `${result.processed} / ${result.total}`
