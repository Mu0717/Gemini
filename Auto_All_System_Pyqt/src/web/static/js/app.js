...
async function loadStats() {
  try {
    ...
    document.getElementById("stat-proxies").textContent =
      stats.available_proxies || 0;
    document.getElementById("stat-cards").textContent =
      stats.available_cards || 0;

    // 更新侧边栏徽章
    document.getElementById("accounts-count").textContent =
      stats.total_accounts || 0;
    document.getElementById("proxies-count").textContent =
...
function updateStatusBars(accountStats) {
  ...
  const statusConfig = {
    ...
    face_check: { label: "人脸验证", color: "#f59e0b" },
...
function renderAccountsTable() {
  ...
  const searchTerm =
    document.getElementById("search-accounts")?.value?.toLowerCase() || "";
  ...
  tbody.innerHTML = filtered
    .map((acc) => {
      const pwdDisplay = state.showSecrets
        ? acc.password || "-"
        : acc.password
          ? "••••••••"
          : "-";
      const secretDisplay = state.showSecrets
        ? acc.secret_key || "-"
        : acc.secret_key
          ? "••••••"
          : "-";
      ...
      return `
        <tr>
            <td>
                <input type="checkbox" class="account-checkbox" data-email="${
                  acc.email
                }"
            ...
            <td>
                <span class="email-cell" onclick="copyToClipboard('${
                  acc.email
                }')" 
            ...
            <td>
                <button class="btn btn-ghost btn-icon-only" onclick="deleteAccount('${
                  acc.email
                }')" title="删除">
                    🗑️
                </button>
...
async function deleteSelectedAccounts() {
  ...
  if (!confirm(`确定删除选中的 ${state.selectedAccounts.size} 个账号？`))
    return;
...
async function exportAccounts() {
  // 获取导出模式
  const exportMode = document.getElementById("export-mode")?.value || "filter";
  const isSelectedMode = exportMode === "selected";

  // 获取通用设置
  const separator =
    document.getElementById("export-accounts-separator")?.value || "----";
  const includeExported =
    document.getElementById("export-include-exported")?.checked ?? true;
  const markExported =
    document.getElementById("export-mark-exported")?.checked ?? true;

  // 获取要导出的字段
  const fields = ["email"]; // 邮箱始终导出
  if (document.getElementById("export-field-password")?.checked)
    fields.push("password");
  if (document.getElementById("export-field-recovery")?.checked)
    fields.push("recovery_email");
  if (document.getElementById("export-field-secret")?.checked)
    fields.push("secret_key");
  if (document.getElementById("export-field-link")?.checked)
    fields.push("verification_link");
  if (document.getElementById("export-field-status")?.checked)
    fields.push("status");
  ...
  if (isSelectedMode) {
    // 导出选中账号模式
    const selectedEmails = Array.from(state.selectedAccounts);
    requestData.emails = selectedEmails;
    filenameSuffix = `_selected_${selectedEmails.length}`;
  } else {
    // 按状态筛选模式
    const statusFilter =
      document.getElementById("export-accounts-status")?.value || "";
    requestData.status = statusFilter;
    filenameSuffix = statusFilter ? `_${statusFilter}` : "";
  }

  try {
    const result = await api.post("/api/accounts/export", requestData);
    ...
    a.href = url;
    a.download = `accounts_export${filenameSuffix}_${new Date()
      .toISOString()
      .slice(0, 10)}.txt`;
    a.click();
...
function exportSelectedAccounts() {
  ...
  // 设置导出模式为"选中"
  document.getElementById("export-mode").value = "selected";
  document.getElementById("export-modal-title").textContent = "导出选中账号";
  document.getElementById("export-selected-info").style.display = "block";
  document.getElementById("export-selected-count").textContent =
    state.selectedAccounts.size;
  document.getElementById("export-status-group").style.display = "none"; // 隐藏状态筛选

  // 打开导出模态框
  showModal("export-accounts");
}
...
function renderProxiesTable() {
  ...
  const searchTerm =
    document.getElementById("search-proxies")?.value?.toLowerCase() || "";
    ...
    .map(
      (p) => `
        <tr>
            ...
            <td>${p.used_by || "-"}</td>
            <td>
                <button class="btn btn-ghost btn-icon-only" onclick="deleteProxy(${
                  p.id
                })" title="删除">
                    🗑️
                </button>
...
function renderCardsTable() {
  ...
  const searchTerm =
    document.getElementById("search-cards")?.value?.toLowerCase() || "";
    ...
    .map((c) => {
      ...
      return `
            <tr>
                ...
                <td>
                    <button class="btn btn-ghost btn-icon-only" onclick="editCard(${
                      c.id
                    })" title="编辑">
                        ✏️
                    </button>
                    <button class="btn btn-ghost btn-icon-only" onclick="toggleCard(${
                      c.id
                    }, ${!c.is_active})" 
                            title="${c.is_active ? "禁用" : "启用"}">
                        ${c.is_active ? "🔒" : "🔓"}
                    </button>
                    <button class="btn btn-ghost btn-icon-only" onclick="deleteCard(${
                      c.id
                    })" title="删除">
                        🗑️
                    </button>
...
async function importCards() {
  ...
  const maxUsage =
    parseInt(document.getElementById("import-cards-max-usage").value) || 1;
  ...
  try {
    const result = await api.post("/api/cards/import", {
      text,
      max_usage: maxUsage,
    });
...
function editCard(id) {
  ...
  document.getElementById("edit-card-active").checked = card.is_active
    ? true
    : false;
...
async function saveCardEdit() {
  ...
  const data = {
    ...
    holder_name:
      document.getElementById("edit-card-holder").value.trim() || null,
    zip_code: document.getElementById("edit-card-zip").value.trim() || null,
    usage_count:
      parseInt(document.getElementById("edit-card-usage").value) || 0,
    max_usage:
      parseInt(document.getElementById("edit-card-max-usage").value) || 1,
    ...
    .map(
      (log) => `
        <div class="log-item">
            ...
            <span class="log-type ${log.operation_type}">${
              log.operation_type
            }</span>
...
async function loadSettings() {
  ...
  try {
    ...
    const apiKey = document
      .getElementById("setting-sheerid_api_key")
      ?.value?.trim();
    if (apiKey) {
...
async function refreshQuota() {
  ...
  try {
    ...
    const maxQuota = 1000;
    const percent =
      quota > 0 ? Math.min(Math.round((quota / maxQuota) * 100), 100) : 0;
...
async function redeemCard() {
  ...
  if (!apiKey) {
    showToast("请先配置并保存 API Key", "warning");
    return;
  }

  try {
    const result = await api.post("/api/sheerid/redeem", {
      code: code,
      api_key: apiKey,
    });
    ...
    if (result.success) {
      showToast(
        `兑换成功！增加 ${result.credits_added} 积分，当前总积分: ${result.credits_total}`,
        "success",
      );
      codeInput.value = "";
      // 刷新配额状态
...
// 系统设置页面 - 链接验证日志辅助函数（表格式布局）
// type: 'info' | 'success' | 'error' | 'warning'
// message: 消息内容
// verificationId: 验证ID（可选，传入后会单独展示在ID列）
function addLinkVerifyLog(message, type = "info", verificationId = null) {
  const logBody = document.getElementById("verify-link-log-body");
  if (!logBody) return;
  ...
  if (verificationId) {
    // 结构化日志行：时间 | 验证ID | 结果badge | 消息
    const badgeMap = {
      success: { text: "✅ 成功", cls: "vb-success" },
      error: { text: "❌ 失败", cls: "vb-error" },
      warning: { text: "⚠️ 警告", cls: "vb-warning" },
      info: { text: "ℹ️ 信息", cls: "vb-info" },
    };
    const badge = badgeMap[type] || badgeMap.info;
    // 截断显示ID
    const shortId =
      verificationId.length > 24
        ? verificationId.substring(0, 24) + "..."
        : verificationId;

    row.className = "vlog-row";
    row.innerHTML = `<span class="vlog-col vlog-col-time">${timeStr}</span><span class="vlog-col vlog-col-id" title="${escapeHtml(verificationId)}" onclick="copyToClipboard('${escapeHtml(verificationId)}')">${escapeHtml(shortId)}</span><span class="vlog-col vlog-col-result"><span class="vlog-badge ${badge.cls}">${badge.text}</span></span><span class="vlog-col vlog-col-msg">${escapeHtml(message)}</span>`;
  } else {
    // 系统消息行（不含验证ID的通用消息）
    row.className = "vlog-row vlog-system";
    row.innerHTML = `<span class="vlog-col vlog-col-time">${timeStr}</span><span class="vlog-sys-msg ${type}">${escapeHtml(message)}</span>`;
  }
...
function clearLinkVerifyLog() {
  const logBody = document.getElementById("verify-link-log-body");
  if (logBody) {
    logBody.innerHTML = '<div class="log-placeholder">等待验证操作...</div>';
  }
}

async function verifySheerIDLink() {
  const input = document.getElementById("verify-link-input");
  const val = input?.value?.trim();
  const apiKeyInput = document.getElementById("setting-sheerid_api_key");
  const apiKey = apiKeyInput?.value?.trim();

  addLinkVerifyLog("开始验证流程...", "info");

  if (!val) {
    addLinkVerifyLog("❌ 未输入链接或 verificationId", "error");
    showToast("请输入链接或 verificationId", "warning");
    return;
  }

  if (!apiKey) {
    addLinkVerifyLog("❌ 未配置 API Key，请先在上方填写并保存", "error");
    showToast("请先配置并保存 API Key", "warning");
    return;
  }

  addLinkVerifyLog(
    `📎 输入内容: ${val.length > 60 ? val.substring(0, 60) + "..." : val}`,
    "info",
  );
  ...
  try {
    ...
    if (params.has("verificationId")) {
      vid = params.get("verificationId");
      addLinkVerifyLog(`🔍 从链接中提取到 verificationId: ${vid}`, "info");
    } else {
      addLinkVerifyLog(
        "⚠️ 链接中未找到 verificationId 参数，将整体作为 ID 使用",
        "warning",
      );
    }
  } catch (e) {
    // 不是 URL，作为 ID 使用
    addLinkVerifyLog(`🔑 作为 Verification ID 使用: ${vid}`, "info");
  }

  try {
    addLinkVerifyLog("📡 正在发送验证请求...", "info");
    showToast("正在验证...", "info");
    ...
    addLinkVerifyLog(`📨 收到服务器响应`, "info");

    if (result.success) {
      const res = result.results?.[0];
      if (res && res.status === "success") {
        addLinkVerifyLog(res.message || "验证成功", "success", vid);
        showToast("验证成功!", "success");
        addLinkVerifyLog("🔄 正在刷新配额信息...", "info");
        setTimeout(refreshQuota, 1000);
      } else {
        const errMsg = res?.message || "未知错误";
        addLinkVerifyLog(errMsg, "error", vid);
        if (res?.details) {
          addLinkVerifyLog(
            `📝 错误详情: ${JSON.stringify(res.details)}`,
            "warning",
          );
        }
        showToast(`验证失败: ${errMsg}`, "error");
      }
    } else {
      const errMsg = result.error || "验证请求失败";
      addLinkVerifyLog(errMsg, "error", vid);
      showToast(errMsg, "error");
    }
  } catch (error) {
    addLinkVerifyLog(error.message || "未知异常", "error", vid);
    showToast(error.message || "系统错误", "error");
  }

  addLinkVerifyLog("--- 验证流程结束 ---", "info");
}
...
function showModal(name) {
  ...
  // 如果是导出账号模态框，重置为筛选模式（除非是从 exportSelectedAccounts 调用）
  if (name === "export-accounts") {
    const exportMode = document.getElementById("export-mode");
    // 只有当 export-mode 不是 'selected' 时才重置（防止覆盖 exportSelectedAccounts 的设置）
    if (exportMode && exportMode.value !== "selected") {
      document.getElementById("export-mode").value = "filter";
      document.getElementById("export-modal-title").textContent = "导出账号";
      document.getElementById("export-selected-info").style.display = "none";
      document.getElementById("export-status-group").style.display = "block";
    }
  }
...
async function loadVerificationAccounts() {
  ...
  try {
    ...
    // 更新统计
    document.getElementById("verify-total").textContent =
      verifyState.accounts.length;
    document.getElementById("verification-count").textContent =
      verifyState.accounts.length;
...
function renderVerificationList() {
  ...
  const html = verifyState.accounts
    .map((acc) => {
      ...
      return `
            <div class="verification-item ${isSelected ? "selected" : ""}" 
                ...
                <input type="checkbox" class="verify-checkbox" 
                       ${isSelected ? "checked" : ""}
                       onchange="toggleVerifySelect('${
                         acc.verification_id
                       }', this)">
                <div class="verify-info">
                    <div class="verify-email">${escapeHtml(acc.email)}</div>
                    <div class="verify-link" title="${escapeHtml(
                      acc.verification_link,
                    )}">
                        ${
                          acc.verification_id
                            ? `ID: ${acc.verification_id.substring(0, 24)}...`
                            : "无验证ID"
                        }
                    </div>
                </div>
  ...
  container.innerHTML =
    html +
    '<div class="empty-state" id="verification-empty" style="display:none;"><span class="empty-icon">📭</span><p>暂无待验证的账号</p><p class="empty-hint">账号状态为"待验证"时会显示在这里</p></div>';
}
...
function updateVerifySelectAllState() {
  ...
  const totalWithId = verifyState.accounts.filter(
    (acc) => acc.verification_id,
  ).length;
  ...
  verifyState.accounts.forEach((acc) => {
    if (acc.verification_id) {
      const item = document.getElementById(
        `verify-item-${acc.verification_id}`,
      );
...
async function startVerification() {
  ...
  try {
    ...
    // 获取 API Key
    const apiKey =
      document.getElementById("setting-sheerid_api_key")?.value?.trim() || "";
    if (!apiKey) {
    ...
    if (result.success) {
      ...
      (result.results || []).forEach((r) => {
        if (r.status === "success") {
          ...
          addVerifyLog(
            "success",
            `${r.verification_id.substring(0, 20)}... 验证成功`,
          );
        } else {
          updateItemStatus(
            r.verification_id,
          ...
          addVerifyLog(
            "error",
            `${r.verification_id.substring(0, 20)}... 失败: ${r.message}`,
          );
        }
      });
      ...
      showToast(
        `验证完成: 成功 ${verifyState.successCount}, 失败 ${verifyState.failedCount}`,
...
function finishVerification() {
  ...
  document.getElementById("verify-success").textContent =
    verifyState.successCount;
  document.getElementById("verify-failed").textContent =
    verifyState.failedCount;

  addVerifyLog(
    "info",
    `验证结束: 成功 ${verifyState.successCount}, 失败 ${verifyState.failedCount}`,
  );
...
// 加载账号列表（支持分页）
async function loadAutoAccounts(page = null) {
  console.log("[AutoProcess] Loading accounts...");

  // 如果指定了页码，更新当前页
  if (page !== null) {
    autoState.pagination.page = page;
  }

  try {
    const result = await api.post("/api/accounts/for_process", {
      page: autoState.pagination.page,
      page_size: autoState.pagination.pageSize,
      status: autoState.statusFilter,
      search: autoState.searchKeyword,
    });
    ...
    if (result.success) {
      ...
      // 更新分页状态
      autoState.pagination.total = result.total || 0;
      autoState.pagination.totalPages = result.total_pages || 1;
      autoState.pagination.hasNext = result.has_next || false;
      autoState.pagination.hasPrev = result.has_prev || false;
      autoState.pagination.page = result.page || 1;

      renderAccountList();
      renderAutoPagination();
      showToast(
        `已加载 ${autoState.accounts.length} 个账号，共 ${autoState.pagination.total} 个`,
        "success",
      );
    } else {
...
function renderAutoPagination() {
  // 查找或创建分页容器
  let paginationContainer = document.getElementById("auto-pagination");
  const listContainer = document.getElementById("browser-list");

  if (!paginationContainer && listContainer) {
    // 在列表容器后面创建分页容器
    paginationContainer = document.createElement("div");
    paginationContainer.id = "auto-pagination";
    paginationContainer.className = "pagination-container";
    listContainer.parentNode.insertBefore(
      paginationContainer,
  ...
  const { page, totalPages, total, hasPrev, hasNext } = autoState.pagination;

  // 如果总数小于等于一页，不显示分页
  if (total <= autoState.pagination.pageSize) {
    paginationContainer.innerHTML = `<div class="pagination-info">共 ${total} 条</div>`;
    return;
  }
  ...
  // 第一页按钮
  if (startPage > 1) {
    pageButtons += `<button class="pagination-btn" onclick="loadAutoAccounts(1)">1</button>`;
    if (startPage > 2) {
  ...
  for (let i = startPage; i <= endPage; i++) {
    pageButtons += `<button class="pagination-btn ${
      i === page ? "active" : ""
    }" onclick="loadAutoAccounts(${i})">${i}</button>`;
  }
  ...
  if (endPage < totalPages) {
    ...
    pageButtons += `<button class="pagination-btn" onclick="loadAutoAccounts(${totalPages})">${totalPages}</button>`;
  }
  ...
  paginationContainer.innerHTML = `
    ...
    <div class="pagination-buttons">
      <button class="pagination-btn" ${
        !hasPrev ? "disabled" : ""
      } onclick="loadAutoAccounts(${page - 1})">上一页</button>
      ${pageButtons}
      <button class="pagination-btn" ${
        !hasNext ? "disabled" : ""
      } onclick="loadAutoAccounts(${page + 1})">下一页</button>
    </div>
  `;
}

// 搜索账号（一键处理）
function searchAutoAccounts() {
  const input = document.getElementById("auto-search-input");
  autoState.searchKeyword = input ? input.value.trim() : "";
  autoState.pagination.page = 1; // 重置到第一页
  loadAutoAccounts();
}

// 筛选状态（一键处理）
function filterAutoAccounts() {
  const select = document.getElementById("auto-status-filter");
  autoState.statusFilter = select ? select.value : "";
  autoState.pagination.page = 1; // 重置到第一页
  loadAutoAccounts();
}
...
function renderAccountList() {
  ...
  container.innerHTML = autoState.accounts
    .map((account) => {
            ...
            <div class="browser-item ${
              isSelected ? "selected" : ""
                ...
                <input type="checkbox" class="browser-checkbox" 
                       ${isSelected ? "checked" : ""} 
                       onclick="event.stopPropagation(); toggleAccountSelect('${
                         account.browser_id
                       }')">
                <div class="browser-info">
                    <div class="browser-email">${escapeHtml(
                      account.email,
                    )}</div>
                    <div class="browser-id">窗口: ${
                      account.browser_id
                        ? account.browser_id.substring(0, 16) + "..."
                        : "未绑定"
                    }</div>
                </div>
                <span class="browser-status ${getStatusClass(
                  displayStatus,
                )}" id="account-status-${account.browser_id}">
...
function toggleAccountSelect(browserId) {
  ...
  if (item) {
    item.classList.toggle(
      "selected",
      autoState.selectedAccounts.has(browserId),
    );
...
function updateAutoSelectedCount() {
  document.getElementById("auto-selected-count").textContent =
    autoState.selectedAccounts.size;

  // 更新全选复选框状态
  const selectAllCheckbox = document.getElementById("auto-select-all");
  if (selectAllCheckbox) {
    const selectableCount = autoState.accounts.filter(
      (a) => a.browser_id,
    ).length;
...
function updateAutoStats() {
  ...
  document.getElementById("auto-processed").textContent =
    autoState.stats.processed;
...
async function startAutoProcess() {
  ...
  if (
    !confirm(
      `确定要处理选中的 ${autoState.selectedAccounts.size} 个账号吗？\n\n流程: 登录 → 资格检测 → SheerID验证 → 绑卡订阅`,
    )
  ) {
    return;
  }
  ...
  const threadCount = parseInt(
    document.getElementById("auto-concurrency")?.value || "1",
    10,
  );
  ...
  addAutoLog(
    "info",
    `开始处理 ${browserIds.length} 个账号，并发数: ${threadCount}...`,
  );
...
async function pollTaskStatus() {
  ...
  try {
    ...
    if (result.success) {
      ...
      // 只有订阅成功才算成功
      autoState.stats.success =
        (stats.subscribed || 0) + (stats.subscribed_antigravity || 0);
      // verified（验证成功但绑卡失败）、link_ready（有链接但未验证）也算失败
      autoState.stats.failed =
        (stats.error || 0) +
        (stats.not_logged_in || 0) +
        (stats.ineligible || 0) +
        (stats.verified || 0) +
        (stats.link_ready || 0);
      ...
      document.getElementById("auto-progress-text").textContent =
        `${result.processed} / ${result.total}`;

      // 更新当前任务
      document.getElementById("current-browser").textContent =
...
function renderAutoResults(results) {
  ...
  grid.innerHTML = results
    .map((r) => {
      ...
      // 生成步骤列表
      const stepsHtml = (r.step_history || [])
        .slice(-5)
        .map((step) => {
          const stepClass =
            step.status === "success"
              ...
              : step.status === "error"
                ? "failed"
                : "";
          return `<div class="result-step ${stepClass}">${
            step.step_display || step.step
          }</div>`;
        })
        .join("");
      ...
      return `
            <div class="result-card ${statusClass}">
                <div class="result-email">${escapeHtml(
                  r.email || r.browser_id,
                )}</div>
                <div class="result-status">${getStatusText(
                  r.final_status,
                )}</div>
...
function addAutoLog(type, message, time = null) {
  ...
  entry.innerHTML = `<span class="log-time">${timeStr}</span>${escapeHtml(
    message,
  )}`;
...
// 加载 SheerLink 账号列表（支持分页）
async function loadSheerLinkAccounts(page = null) {
  console.log("[SheerLink] Loading accounts...");

  // 如果指定了页码，更新当前页
  if (page !== null) {
    sheerlinkState.pagination.page = page;
  }

  try {
    const result = await api.post("/api/accounts/for_process", {
      page: sheerlinkState.pagination.page,
      page_size: sheerlinkState.pagination.pageSize,
      status: sheerlinkState.statusFilter,
      search: sheerlinkState.searchKeyword,
    });
    ...
    if (result.success) {
      ...
      // 更新分页状态
      sheerlinkState.pagination.total = result.total || 0;
      sheerlinkState.pagination.totalPages = result.total_pages || 1;
      sheerlinkState.pagination.hasNext = result.has_next || false;
      sheerlinkState.pagination.hasPrev = result.has_prev || false;
      sheerlinkState.pagination.page = result.page || 1;

      renderSheerLinkList();
      renderSheerLinkPagination();
      showToast(
        `已加载 ${sheerlinkState.accounts.length} 个账号，共 ${sheerlinkState.pagination.total} 个`,
        "success",
      );
    }
  } catch (error) {
...
// 渲染 SheerLink 分页控件
function renderSheerLinkPagination() {
  // 查找或创建分页容器
  let paginationContainer = document.getElementById("sheerlink-pagination");
  const listContainer = document.getElementById("sheerlink-list");

  if (!paginationContainer && listContainer) {
    // 在列表容器后面创建分页容器
    paginationContainer = document.createElement("div");
    paginationContainer.id = "sheerlink-pagination";
    paginationContainer.className = "pagination-container";
    listContainer.parentNode.insertBefore(
      paginationContainer,
      listContainer.nextSibling,
  ...
  const { page, totalPages, total, hasPrev, hasNext } =
    sheerlinkState.pagination;

  // 如果总数小于等于一页，不显示分页
  if (total <= sheerlinkState.pagination.pageSize) {
    paginationContainer.innerHTML = `<div class="pagination-info">共 ${total} 条</div>`;
    return;
  }
  ...
  // 第一页按钮
  if (startPage > 1) {
    pageButtons += `<button class="pagination-btn" onclick="loadSheerLinkAccounts(1)">1</button>`;
    if (startPage > 2) {
      pageButtons += `<span class="pagination-ellipsis">...</span>`;
    }
  }
  ...
  for (let i = startPage; i <= endPage; i++) {
    pageButtons += `<button class="pagination-btn ${
      i === page ? "active" : ""
    }" onclick="loadSheerLinkAccounts(${i})">${i}</button>`;
  }
  ...
  if (endPage < totalPages) {
    ...
    pageButtons += `<button class="pagination-btn" onclick="loadSheerLinkAccounts(${totalPages})">${totalPages}</button>`;
  }
  ...
  paginationContainer.innerHTML = `
    ...
    <div class="pagination-buttons">
      <button class="pagination-btn" ${
        !hasPrev ? "disabled" : ""
      } onclick="loadSheerLinkAccounts(${page - 1})">上一页</button>
      ${pageButtons}
      <button class="pagination-btn" ${
        !hasNext ? "disabled" : ""
      } onclick="loadSheerLinkAccounts(${page + 1})">下一页</button>
    </div>
  `;
}

// 搜索 SheerLink 账号
function searchSheerLinkAccounts() {
  const input = document.getElementById("sheerlink-search-input");
  sheerlinkState.searchKeyword = input ? input.value.trim() : "";
  sheerlinkState.pagination.page = 1; // 重置到第一页
  loadSheerLinkAccounts();
}

// 筛选 SheerLink 状态
function filterSheerLinkAccounts() {
  const select = document.getElementById("sheerlink-status-filter");
  sheerlinkState.statusFilter = select ? select.value : "";
  sheerlinkState.pagination.page = 1; // 重置到第一页
  loadSheerLinkAccounts();
}

// 一键勾选未验证账号（link_ready 和 pending_check 状态）
function selectUnverifiedSheerLink() {
  if (sheerlinkState.isRunning) return;

  // 清空当前选择
  sheerlinkState.selectedAccounts.clear();

  // 勾选所有未验证状态的账号（link_ready 或 pending_check）
  const unverifiedStatuses = ["link_ready", "pending_check"];
  sheerlinkState.accounts.forEach((acc) => {
    if (acc.browser_id && unverifiedStatuses.includes(acc.status)) {
      sheerlinkState.selectedAccounts.add(acc.browser_id);
    }
  });

  renderSheerLinkList();
  const count = sheerlinkState.selectedAccounts.size;
  showToast(`已勾选 ${count} 个未验证账号`, "info");
}
...
function renderSheerLinkList() {
  ...
  if (sheerlinkState.accounts.length === 0) {
    container.innerHTML =
      '<div class="empty-state"><span class="empty-icon">📭</span><p>暂无账号</p></div>';
    return;
  }

  container.innerHTML = sheerlinkState.accounts
    .map((acc) => {
      ...
      return `
            <div class="browser-item ${
              isSelected ? "selected" : ""
                 ...
                 data-id="${acc.browser_id}" onclick="toggleSheerLinkSelect('${
                   acc.browser_id
                 }')">
                <input type="checkbox" class="browser-checkbox" ${
                  isSelected ? "checked" : ""
                } 
                       onclick="event.stopPropagation(); toggleSheerLinkSelect('${
                         acc.browser_id
                       }')">
                <div class="browser-info">
                    ...
                    <div class="browser-id">窗口: ${
                      acc.browser_id
                        ? acc.browser_id.substring(0, 16) + "..."
                        : "未绑定"
                    }</div>
                </div>
                <span class="browser-status ${statusClass}">${getStatusText(
                  acc.status,
                )}</span>
            </div>
        `;
...
function updateSheerLinkSelectedCount() {
  document.getElementById("sheerlink-selected-count").textContent =
    sheerlinkState.selectedAccounts.size;
}
...
async function startSheerLinkProcess() {
  ...
  if (
    !confirm(
      `确定要提取选中的 ${sheerlinkState.selectedAccounts.size} 个账号的 SheerLink 吗？`,
    )
  ) {
    return;
  }
  ...
  const concurrency =
    parseInt(document.getElementById("sheerlink-concurrency").value) || 1;
  ...
  addSheerLinkLog(
    "info",
    `开始提取 ${browserIds.length} 个账号，并发: ${concurrency}`,
  );
...
async function pollSheerLinkStatus() {
  ...
  try {
    const result = await api.post("/api/task/status", {
      task_id: sheerlinkState.taskId,
    });
    ...
    if (result.success) {
      ...
      (result.results || []).forEach((r) => {
        ...
        if (
          msg.includes("link found") ||
          msg.includes("待验证") ||
          msg.includes("link_ready")
        ) {
          stats.link_ready++;
        } else if (
          msg.includes("verified") ||
          msg.includes("已验证") ||
          msg.includes("get offer")
        ) {
          stats.verified++;
        } else if (
      ...
      const percent =
        result.total > 0
          ? Math.round((result.processed / result.total) * 100)
          : 0;
      document.getElementById("sheerlink-progress-bar").style.width =
        `${percent}%`;
      document.getElementById("sheerlink-progress-text").textContent =
        `${result.processed} / ${result.total}`;
...
function updateSheerLinkStats() {
  ...
  document.getElementById("sheerlink-face").textContent = s.face || 0;
...
function addSheerLinkLog(type, message) {
  ...
  const entry = document.createElement("div");
  entry.className = `log-entry ${type}`;
  entry.innerHTML = `<span class="log-time">${time}</span>${escapeHtml(
    message,
  )}`;
  logContainer.appendChild(entry);
  logContainer.scrollTop = logContainer.scrollHeight;
...
function clearSheerLinkLog() {
  document.getElementById("sheerlink-log").innerHTML =
...
function renderBindCardList() {
  ...
  if (bindcardState.accounts.length === 0) {
    container.innerHTML =
      '<div class="empty-state"><span class="empty-icon">📭</span><p>暂无已验证账号</p></div>';
    return;
  }

  container.innerHTML = bindcardState.accounts
    .map((acc) => {
      const isSelected = bindcardState.selectedAccounts.has(acc.browser_id);

      return `
            <div class="browser-item ${isSelected ? "selected" : ""}" 
                 data-id="${acc.browser_id}" onclick="toggleBindCardSelect('${
                   acc.browser_id
                 }')">
                <input type="checkbox" class="browser-checkbox" ${
                  isSelected ? "checked" : ""
                } 
                       onclick="event.stopPropagation(); toggleBindCardSelect('${
                         acc.browser_id
                       }')">
                <div class="browser-info">
                    <div class="browser-email">${escapeHtml(acc.email)}</div>
...
function updateBindCardSelectedCount() {
  document.getElementById("bindcard-selected-count").textContent =
    bindcardState.selectedAccounts.size;
}
...
async function startBindCardProcess() {
  ...
  if (
    !confirm(
      `确定要为选中的 ${bindcardState.selectedAccounts.size} 个账号绑卡订阅吗？`,
    )
  ) {
    return;
  }
  ...
  const concurrency =
    parseInt(document.getElementById("bindcard-concurrency").value) || 1;
  ...
  addBindCardLog(
    "info",
    `开始绑卡 ${browserIds.length} 个账号，并发: ${concurrency}`,
  );
    ...
    if (result.success) {
      ...
      const percent =
        result.total > 0
          ? Math.round((result.processed / result.total) * 100)
          : 0;
      document.getElementById("bindcard-progress-bar").style.width =
        `${percent}%`;
      document.getElementById("bindcard-progress-text").textContent =
        `${result.processed} / ${result.total}`;
...
function updateBindCardStats() {
  document.getElementById("bindcard-total").textContent =
    bindcardState.stats.total;
  document.getElementById("bindcard-processed").textContent =
    bindcardState.stats.processed;
  document.getElementById("bindcard-success").textContent =
    bindcardState.stats.success;
  document.getElementById("bindcard-failed").textContent =
    bindcardState.stats.failed;
}
...
function clearBindCardLog() {
  document.getElementById("bindcard-log").innerHTML =
    '<div class="log-empty">等待开始...</div>';
}
...
// 加载 Change2FA 账号列表（支持分页）
async function loadChange2FAAccounts(page = null) {
  console.log("[Change2FA] Loading accounts...");

  // 如果指定了页码，更新当前页
  if (page !== null) {
    change2faState.pagination.page = page;
  }

  try {
    // 获取有2FA密钥的账号
    const result = await api.post("/api/accounts/for_process", {
      page: change2faState.pagination.page,
      page_size: change2faState.pagination.pageSize,
      status: change2faState.statusFilter,
      search: change2faState.searchKeyword,
      has_twofa: true, // 只获取有2FA密钥的账号
    });

    if (result.success) {
      change2faState.accounts = result.accounts || [];
      change2faState.selectedAccounts.clear();

      // 更新分页状态
      change2faState.pagination.total = result.total || 0;
      change2faState.pagination.totalPages = result.total_pages || 1;
      change2faState.pagination.hasNext = result.has_next || false;
      change2faState.pagination.hasPrev = result.has_prev || false;
      change2faState.pagination.page = result.page || 1;

      renderChange2FAList();
      renderChange2FAPagination();
      showToast(
        `已加载 ${change2faState.accounts.length} 个账号，共 ${change2faState.pagination.total} 个`,
        "success",
      );
    }
  } catch (error) {
...
// 渲染 Change2FA 分页控件
function renderChange2FAPagination() {
  // 查找或创建分页容器
  let paginationContainer = document.getElementById("change2fa-pagination");
  const listContainer = document.getElementById("change2fa-list");

  if (!paginationContainer && listContainer) {
    // 在列表容器后面创建分页容器
    paginationContainer = document.createElement("div");
    paginationContainer.id = "change2fa-pagination";
    paginationContainer.className = "pagination-container";
    listContainer.parentNode.insertBefore(
      paginationContainer,
      listContainer.nextSibling,
  ...
  const { page, totalPages, total, hasPrev, hasNext } =
    change2faState.pagination;

  // 如果总数小于等于一页，不显示分页
  if (total <= change2faState.pagination.pageSize) {
    paginationContainer.innerHTML = `<div class="pagination-info">共 ${total} 条</div>`;
    return;
  }
  ...
  // 第一页按钮
  if (startPage > 1) {
    pageButtons += `<button class="pagination-btn" onclick="loadChange2FAAccounts(1)">1</button>`;
    if (startPage > 2) {
      pageButtons += `<span class="pagination-ellipsis">...</span>`;
    }
  }
  ...
  for (let i = startPage; i <= endPage; i++) {
    pageButtons += `<button class="pagination-btn ${
      i === page ? "active" : ""
    }" onclick="loadChange2FAAccounts(${i})">${i}</button>`;
  }
  ...
  if (endPage < totalPages) {
    ...
    pageButtons += `<button class="pagination-btn" onclick="loadChange2FAAccounts(${totalPages})">${totalPages}</button>`;
  }
  ...
  paginationContainer.innerHTML = `
    ...
    <div class="pagination-buttons">
      <button class="pagination-btn" ${
        !hasPrev ? "disabled" : ""
      } onclick="loadChange2FAAccounts(${page - 1})">上一页</button>
      ${pageButtons}
      <button class="pagination-btn" ${
        !hasNext ? "disabled" : ""
      } onclick="loadChange2FAAccounts(${page + 1})">下一页</button>
    </div>
  `;
}

// 搜索 Change2FA 账号
function searchChange2FAAccounts() {
  const input = document.getElementById("change2fa-search-input");
  change2faState.searchKeyword = input ? input.value.trim() : "";
  change2faState.pagination.page = 1; // 重置到第一页
  loadChange2FAAccounts();
}

// 筛选 Change2FA 状态
function filterChange2FAAccounts() {
  const select = document.getElementById("change2fa-status-filter");
  change2faState.statusFilter = select ? select.value : "";
  change2faState.pagination.page = 1; // 重置到第一页
  loadChange2FAAccounts();
}
...
function renderChange2FAList() {
  ...
  if (change2faState.accounts.length === 0) {
    container.innerHTML =
      '<div class="empty-state"><span class="empty-icon">📭</span><p>暂无有2FA密钥的账号</p></div>';
    return;
  }

  container.innerHTML = change2faState.accounts
    .map((acc) => {
      ...
      return `
            <div class="browser-item ${
              isSelected ? "selected" : ""
                 ...
                 data-id="${acc.browser_id}" onclick="toggleChange2FASelect('${
                   acc.browser_id
                 }')">
                <input type="checkbox" class="browser-checkbox" ${
                  isSelected ? "checked" : ""
                } 
                       onclick="event.stopPropagation(); toggleChange2FASelect('${
                         acc.browser_id
                       }')">
                <div class="browser-info">
                    <div class="browser-email">${escapeHtml(acc.email)}</div>
                    <div class="browser-id">2FA: ${
                      acc.twofa_key
                        ? acc.twofa_key.substring(0, 8) + "..."
                        : "无"
                    }</div>
                </div>
                <span class="browser-status ${statusClass}">${getStatusText(
...
function updateChange2FASelectedCount() {
  document.getElementById("change2fa-selected-count").textContent =
    change2faState.selectedAccounts.size;
}
...
async function startChange2FAProcess() {
  ...
  if (
    !confirm(
      `确定要更改选中的 ${change2faState.selectedAccounts.size} 个账号的2FA密钥吗？\n\n⚠️ 注意：更改后原2FA密钥将失效！`,
    )
  ) {
    return;
  }
  ...
  const concurrency =
    parseInt(document.getElementById("change2fa-concurrency").value) || 1;
  ...
  addChange2FALog(
    "info",
    `开始更改 ${browserIds.length} 个账号的2FA密钥，并发: ${concurrency}`,
  );
    ...
    if (result.success) {
      ...
      const percent =
        result.total > 0
          ? Math.round((result.processed / result.total) * 100)
          : 0;
      document.getElementById("change2fa-progress-bar").style.width =
        `${percent}%`;
      document.getElementById("change2fa-progress-text").textContent =
        `${result.processed} / ${result.total}`;
...
function updateChange2FAStats() {
  document.getElementById("change2fa-total").textContent =
    change2faState.stats.total;
  document.getElementById("change2fa-processed").textContent =
    change2faState.stats.processed;
  document.getElementById("change2fa-success").textContent =
    change2faState.stats.success;
  document.getElementById("change2fa-failed").textContent =
    change2faState.stats.failed;
}
...
function clearChange2FALog() {
  document.getElementById("change2fa-log").innerHTML =
    '<div class="log-empty">等待开始...</div>';
}
